...
class GuardianFetcher(BaseNewsFetcher):
    ...
    async def fetch(self, keywords=None, max_results=50, days_back=7) -> AsyncIterator[NewsData]:
        ...
        for entry in feed.entries[:max_results]:
            ...
            yield NewsData(title=title, summary=entry.get("summary"), source=self.source_name, source_id=entry.get("id", ""), url=entry.get("link"), published_date=pub_date, author="The Guardian", category=self.category, tags=["guardian", "world"], raw_data={})
